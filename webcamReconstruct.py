...
# --- [수정] 웹캠 캡처 및 '자동' 연속 재구성 함수 ---
def opencv_capture_and_reconstruct():
    """
    웹캠을 제어하고, 이전 재구성이 완료되면 자동으로 다음 프레임을 캡처하여
    연속적으로 3D 재구성을 수행합니다.
    """
    ...
    # 안내 메시지 변경
    print("\n자동 연속 3D 재구성을 시작합니다.")
    print("OpenCV 창에서 'q' 키를 누르면 프로그램이 종료됩니다.")

    # 메인 루프: 키 입력 대기 없이 연속 실행
    while True:
        # 1. 최신 프레임 가져오기 및 화면에 표시
        frames = [cap.read() for cap in caps]
        valid_frames = [frame for ret, frame in frames if ret]

        if len(valid_frames) != len(caps):
            print("오류: 일부 웹캠에서 프레임을 가져올 수 없습니다. 루프를 중단합니다.")
            break

        for i, frame in enumerate(valid_frames):
            cv2.imshow(f"Webcam ID: {WEBCAM_INDICES[i]}", frame)

        # 2. 종료 키('q') 확인 (imshow 창 유지를 위해 필수)
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print("프로그램을 종료합니다.")
            break
        
        # 3. 스페이스바 대기 없이 바로 재구성 로직 실행
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        print(f"\n[{timestamp}] 다음 재구성 사이클 시작:")
        ...
        print(f"  - {len(valid_frames)}개의 새 이미지를 저장했습니다.")
        
        # 재구성 함수 호출 (이 함수가 끝날 때까지 여기서 대기)
        perform_reconstruction(FIXED_TARGET_DIR)
        
        # 재구성이 끝나면 루프의 처음으로 돌아가 바로 다음 프레임을 캡처
